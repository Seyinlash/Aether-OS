/* Storage controls use the same service and usage figures as File Manager. */
(() => {
  'use strict';
  ...
  OS.StorageSettings = Object.freeze({ launch(services) {
    ...
    $('.import-data').addEventListener('change',()=>run(async()=>{const file=$('.import-data').files[0];if(!file)return;try{if(file.size>50*1024*1024)throw new Error('Choose an Aether JSON backup smaller than 50 MB.');const backup=storage.validateBackup(JSON.parse(await file.text()));if(!confirm(`Replace all Aether files and preferences in this browser with this backup (${backup.nodes.filter(n=>n.type==='file').length} files)? Existing Aether data will be deleted. Export first if you want to keep it.`))return;await storage.importData(backup);location.reload();}finally{$('.import-data').value='';}}));
    $('.reset-data').addEventListener('click',()=>run(async()=>{if(!confirm('Reset Aether OS? This permanently deletes ALL Aether virtual files, application data, and settings in this browser. Your computer files are not touched. Export a backup first if needed.'))return;await storage.reset();location.reload();}));
    const unsubscribe=storage.subscribe(refresh);refresh();return{content,dispose(){disposed=true;unsubscribe();}};
  }});
})();