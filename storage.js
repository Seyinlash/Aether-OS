...
(() => {
  ...
  // Atomic app-data updates keep read/modify/write operations safe across windows.
  async function updateData(key, updater) { const database = await db(); return new Promise((resolve,reject)=>{const tx=database.transaction('content','readwrite'),store=tx.objectStore('content');let result,failure;const request=store.get(key);request.onsuccess=()=>{try{result=updater(request.result);if(result&&typeof result.then==='function')throw Error('Data updater must be synchronous');result=JSON.parse(JSON.stringify(result));store.put(result,key);}catch(error){failure=error;tx.abort();}};tx.oncomplete=()=>{changed();resolve(result);};tx.onabort=tx.onerror=()=>reject(failure||new Error('Application data could not be saved.'));}); }
  ...
  OS.Storage = Object.freeze({ settings, fs, ready, get wasReplaced() { return wasReplaced; },
    data: Object.freeze({ update:updateData, get:key => dataOperation('get',key), set:(key,value) => { if (typeof key !== 'string') throw new Error('Use a string data key.'); JSON.stringify(value); return dataOperation('put',key,JSON.parse(JSON.stringify(value))); }, delete:key => dataOperation('delete',key) }),
